...
const { corsHeadersFor } = require('./client-gate');
...
function jsonResponse(body, status = 200, extra = {}, origin = '*') {
 ...
 */
async function handleAppApi(request, reqUrl, opts = {}) {
    const api = String(reqUrl.searchParams.get('api') || '').trim();
    if (!api) return null;
    const origin = opts.origin || '*';
    const reply = (body, status = 200, extra = {}) => jsonResponse(body, status, extra, origin);
    ...
    try {
        if (api === 'health') {
            return reply({
                ...
                apis: [
                    ...
                    'instructions-catalog', 'instructions-chunk', 'instructions-search', 'instructions-file',
                ],
            });
        }

        if (api === 'spr') {
            return reply(readJsonFile('spr.json'));
        }
        if (api === 'trains-local') {
            return reply(readJsonFile('trains-local.json'));
        }
        if (api === 'shift-templates') {
            return reply(readJsonFile('shift-templates.json'));
        }
        if (api === 'calendar-local-routes') {
            return reply(readJsonFile('calendar-local-routes.json'));
        }
        if (api === 'line-sections') {
            return reply(readJsonFile('line-sections.json'));
        }
        ...
        if (api === 'brakes') {
            ...
            return reply({ ok: true, station, direction: direction || null, brakes });
        }
        ...
        if (api === 'shift-template') {
            ...
            return reply({ ok: true, route, date, template });
        }
        ...
        if (api === 'instructions-catalog') {
            return reply(readJsonFile(path.join('instructions', 'catalog.json')));
        }

        if (api === 'instructions-chunk') {
            const id = String(reqUrl.searchParams.get('id') || '').replace(/[^a-zA-Z0-9_-]/g, '');
            if (!id) return reply({ ok: false, error: 'id required' }, 400);
            const file = path.join(DATA_DIR, 'instructions', 'chunks', `${id}.json`);
            if (!fs.existsSync(file)) return reply({ ok: false, error: 'not found' }, 404);
            return reply(JSON.parse(fs.readFileSync(file, 'utf8')));
        }
        ...
        if (api === 'instructions-search') {
            ...
            return reply({ ok: true, query: q, results });
        }

        if (api === 'instructions-file') {
            const id = String(reqUrl.searchParams.get('id') || '').replace(/[^a-zA-Z0-9_-]/g, '');
            if (!id) return reply({ ok: false, error: 'id required' }, 400);
            const candidates = [
                path.join(DATA_DIR, 'instructions', 'pdf', `${id}.pdf`),
                path.join(DATA_DIR, 'instructions', 'files', `${id}.pdf`),
                path.join(__dirname, '..', '..', 'assets', 'instructions-pdf', `${id}.pdf`),
            ];
        ...
        return reply({ ok: false, error: `unknown api: ${api}` }, 400);
    } catch (err) {
        console.error('[app-api]', api, err);
        return reply({ ok: false, error: err.message || String(err) }, 500);
    }
}
